...
WSFrameMessage::ptr WSRecvMessage(Stream* stream, bool isClient) {
    ...
    do {
        ...
        if(stream->readFixSize(&head, sizeof(head)) <= 0) {
            // SYLAR_LOG_INFO(g_logger) << "readFixSize";
            break;
        }
        SYLAR_LOG_DEBUG(g_logger) << "WSFrameHead " << head.toString();

        // 如果收到的是ping包，则发送pong包
        if(head.opcode == WSFrameHead::PING) {
            SYLAR_LOG_INFO(g_logger) << "PING";
            if(WSPong(stream) <= 0) {
                // SYLAR_LOG_INFO(g_logger) << "WSPong";
                break;
            }
        }
            ...
            if(head.payload == 126) {
                ...
                if(stream->readFixSize(&len, sizeof(len)) <= 0) {
                    // SYLAR_LOG_INFO(g_logger) << "readFixSize2";
                    break;
                }
                ...
                uint64_t len = 0;
                if(stream->readFixSize(&len, sizeof(len)) <= 0) {
                    // SYLAR_LOG_INFO(g_logger) << "readFixSize3";
                    break;
                }
                length = byteSwapToLittleEndian(len);
            } 
            ...
            if(head.mask) {
                if(stream->readFixSize(mask, sizeof(mask)) <= 0) {
                    // SYLAR_LOG_INFO(g_logger) << "readFixSize4";
                    break;
                }
            }
            ...
            if(stream->readFixSize(&data[curr_len], length) <= 0) {
                // SYLAR_LOG_INFO(g_logger) << "readFixSize5";
                break;
            }
            ...
            SYLAR_LOG_INFO(g_logger) << "invalid opcode=" << head.opcode;
            // SYLAR_LOG_INFO(g_logger) << "invalid opcode=" << head.toString();
            break;
        }
    } while(true);
    stream->close();
